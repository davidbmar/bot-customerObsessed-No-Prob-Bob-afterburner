"""Google Sign-In authentication — JWT verification and session management.

Verifies Google Identity Services JWTs and manages session tokens.
Ported from iphone-and-companion-transcribe-mode/gateway/auth.py.
"""
...
def verify_google_jwt(jwt_token: str) -> dict:
    ...
    Raises ValueError if the token is invalid, expired, or unauthorized.
    """
...
def authenticate_google(jwt_token: str) -> tuple[int, str, dict]:
    ...
    """
    from .db import upsert_user, create_auth_session
...
def authenticate_session_token(token: str) -> tuple[int, dict] | None:
    ...
    """
    from .db import validate_auth_session, get_user
